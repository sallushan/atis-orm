...
namespace Atis.Orm.Metadata
{
    ...
    ///     </para>
    ///     <para>
    ///         Single-entity (<c>Func&lt;TOther&gt;</c>) navigations are served from a short-lived cache:
    ///         a loaded entity is reused for repeated calls within <see cref="CacheExpiration"/>, but
    ///         only while the join keys still match (the compiled <see cref="NavigationInfo.JoinCondition"/>
    ///         is re-evaluated against the caller, so changing a foreign key forces a reload). A sweep
    ///         timer evicts expired entries so a long-lived initializer does not pin entities that went
    ///         out of scope; disposing the DI scope disposes the timer.
    ///     </para>
    ///     <para>
    ...
    ///     </para>
    /// </summary>
    public class NavigationInitializer2 : INavigationInitializer, IDisposable
    {
        private static MethodInfo _getChildrenQueryMethod;
        private static MethodInfo GetChildrenQueryMethod =>
            _getChildrenQueryMethod ??
            (_getChildrenQueryMethod = typeof(NavigationInitializer2).GetMethod(nameof(GetChildrenQuery), BindingFlags.NonPublic | BindingFlags.Instance));

        private static MethodInfo _getOtherEntityMethod;
        private static MethodInfo GetOtherEntityMethod =>
            _getOtherEntityMethod ??
            (_getOtherEntityMethod = typeof(NavigationInitializer2).GetMethod(nameof(GetOtherEntity), BindingFlags.NonPublic | BindingFlags.Instance));
        ...
        /// </summary>
        private static readonly ConcurrentDictionary<Type, Action<NavigationInitializer2, object>> iteratorCache
            = new ConcurrentDictionary<Type, Action<NavigationInitializer2, object>>();
        ...
        /// </summary>
        private readonly ConcurrentDictionary<NavigationInfo, SingleEntityCacheEntry> singleEntityCache
            = new ConcurrentDictionary<NavigationInfo, SingleEntityCacheEntry>();
        ...
        /// </summary>
        public NavigationInitializer2(IQueryableFactory queryableFactory, IOrmModel model, IReflectionService reflectionService)
        {
        ...
        /// <summary>
        ///     How long a cached single-entity navigation result stays valid. Short by design: the cache
        ///     only exists to absorb repeated navigation calls within one code flow.
        /// </summary>
        ...
        /// </summary>
        private Action<NavigationInitializer2, object> CreateNavigationPropertiesIterator(Type entityType, EntityMetadata metadata)
        {
            var initializerParam = Expression.Parameter(typeof(NavigationInitializer2), "initializer");
            ...
            return Expression.Lambda<Action<NavigationInitializer2, object>>(block, initializerParam, entityObjParam).Compile();
        }
        ...
        private TOtherEntity GetOtherEntity<TOtherEntity>(object thisEntity, NavigationInfo navigation)
        {
            ...
            if (this.singleEntityCache.TryGetValue(navigation, out var entry)
                && now - entry.CreatedAtUtc < this.CacheExpiration
            ...
            if (otherEntity != null)
            {
                this.singleEntityCache[navigation] = new SingleEntityCacheEntry(otherEntity, GetOrAddMatchFunction(navigation), now);
                this.EnsureSweepTimerStarted();
            }
            return otherEntity;
        }
        ...
        ///         registers that type's metadata), and <see cref="NavigationInfo.JoinCondition"/> —
        ///         when present — is applied on top as a <c>Where</c> correlating the source to this
        ///         entity.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     Compiles the join condition into <c>(thisEntity, otherEntity) =&gt; bool</c> — returns
        ...
        private void EnsureSweepTimerStarted()
        {
            if (this.sweepTimer != null || this.disposed)
                return;
            lock (this.singleEntityCache)
            {
                if (this.sweepTimer != null || this.disposed)
                    return;
                var period = this.CacheExpiration;
                this.sweepTimer = new Timer(this.SweepExpiredEntries, null, period, period);
            }
        }
        ...
        {
            var now = DateTime.UtcNow;
            foreach (var pair in this.singleEntityCache)
            {
                if (now - pair.Value.CreatedAtUtc >= this.CacheExpiration)
                    this.singleEntityCache.TryRemove(pair.Key, out _);
            }
        }
        ...
        public void Dispose()
        {
            ...
            this.singleEntityCache.Clear();
        }

        private sealed class SingleEntityCacheEntry
        {
            public SingleEntityCacheEntry(object otherEntity, Func<object, object, bool> matchFunction, DateTime createdAtUtc)
            {
        ...
        {
            private readonly NavigationInitializer2 initializer;

            public QueryRootReplacementVisitor(NavigationInitializer2 initializer)
            {
